...
Quotas, the monthly import window and the protocol are described in [docs/registry-bridge.md](docs/registry-bridge.md); the terms are at [cellarion.app/terms](https://cellarion.app/terms).
...
| OIDC SSO | `OIDC_CLIENT_ID`, `OIDC_CLIENT_SECRET`, `OIDC_AUTHORIZATION_URL`, `OIDC_TOKEN_URL`, `OIDC_USERINFO_URL`, `OIDC_ISSUER`, `OIDC_PROVIDER_NAME`, `OIDC_CALLBACK_URL`, `OIDC_SCOPES`, `OIDC_TRUST_EMAIL_VERIFIED` | Sign in with your own identity provider (Pocket ID, Authentik, Keycloak, Zitadel, Authelia). Works alongside Google; endpoints come from the provider's `/.well-known/openid-configuration` |
