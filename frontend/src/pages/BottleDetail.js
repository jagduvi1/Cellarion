      ...
      {wine?.aiProfile?.description && (
        <div className="bd-ai-profile card">
          <div className="bd-ai-profile__header">
            ...
            <span className="bd-ai-tag" title={t('bottleDetail.aiGeneratedHint', 'Generated by AI — may not be exact')}>
              {t('bottleDetail.aiGenerated', 'AI-generated')}
            </span>
          </div>
          ...
          {(() => {
            ...
            return (chips.length > 0 || ap.flavors?.length > 0) ? (
              <div className="bd-ai-chips">
                {chips.map((c, i) => <span key={`s${i}`} className="bd-ai-chip bd-ai-chip--style">{c}</span>)}
                {(ap.flavors || []).map((f, i) => <span key={`f${i}`} className="bd-ai-chip">{f}</span>)}
              </div>
            ) : null;
          })()}
