...
function ImportBottles() {
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      // Auto-select exact, fuzzy, and AI-identified matches. AI-identified NEW
      // wines default to 'create' — the wine is minted at /confirm, so leaving
      // the row unselected would silently drop it from the import.
      const autoSelections = {};
      allResults.forEach((r) => {
        if ((r.status === 'exact' || r.status === 'fuzzy' || r.status === 'ai_match') && r.matches.length > 0) {
          autoSelections[r.index] = r.matches[0].wineId;
        } else if (r.status === 'ai_new' && r.aiProposed) {
          autoSelections[r.index] = 'create';
        }
      });
