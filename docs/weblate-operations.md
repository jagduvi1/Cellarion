...
2. **Is Weblate pushing at all?** Judge by DATES, not percentages — the two sides count against different English baselines, so a gap between them means nothing on its own (§7). Look at when the bot last opened a PR, and at the component's own state: open the component → **Operations → Repository maintenance** (the same dropdown as Update — there is no Manage menu, see §3) and read the alert. The usual causes are a rebase conflict after a squash-merge (plain **Update** fixes it) or a locked component. Never guess at "Reset" — §7 explains which one is right.
...
- **A "Reset" discards Weblate's local git state, and which reset is safe depends on which side actually holds newer work — which is NOT what the percentages tell you.**
  - **Read the trap first.** Weblate's percentage is measured against the English base *in its own checkout*, and the app's against the English in `main`. When Weblate has fallen behind, its denominator is smaller and its percentages are therefore **higher**, which looks exactly like unpushed work and is not. On 2026-09-09 Weblate showed fr 99 / de 99 / sv 95 % against the app's 83 / 83 / 81 %, and the honest reading was the opposite of the obvious one: its English base was 3,433 keys against `main`'s 4,357. Exporting the whole project and diffing it proved Weblate held **one stale string per language and seven new Estonian ones** — nothing else that `main` did not already have.
  - **To find out for real, compare content, not percentages.** **Files → Download translation files as ZIP**, then diff each language against `frontend/src/locales/`. It is a read, it works while the project is locked, and it answers the question outright. `git log --format='%h %ad %s' <last-l10n-commit>..origin/main -- frontend/src/locales/<lang>` shows the other direction: whether anything but Weblate has touched those files (a feature PR removing keys `en` dropped is normal; anything adding translations is not).
  - **Repository newer or equal** — the usual case, and the 2026-09-09 one: **"Reset and discard"**. Upstream is the truth. Export first anyway, land anything genuinely new as its own PR, and then nothing is at stake.
  - **Weblate genuinely newer** (translators worked, Weblate could not push): "Reset and discard" would throw that away. Use **Update**; if it cannot rebase, **"Reset and reapply"** rewrites the files from the database on top of current `main`. Expect a large PR and let CI judge it — the locale tests and the auto-merge workflow's 500-deletion guard are what catch a bad one. This is the case the 2026-07-23 incident is remembered for, in reverse: back then "Reset and reapply" wrote 3,195 stale database units over newer `main`, resurrecting deleted keys and blanking plurals.
