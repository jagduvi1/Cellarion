...
| **Returning users (retention)** | Two day-ladders (2/4/7 distinct days) — one on bottle activity, one on logins — plus single-session users and the login windows (logged-in 30d/7d, repeat logins) | `Bottle.aggregate` (distinct user×day) + `AuditLog.aggregate` on `LOGIN_ACTIONS` (distinct user×day) |
